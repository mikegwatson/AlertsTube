...
from pigredients.ics import ws2801 as ws2801			# Using forked version of https://github.com/rasathus/pigredients
...
ALERT_SEVERITY = "minor"   # Options: "severe" | "major" | "moderate" | "minor", determines when an alert will be triggered 
...
def fetch_weather(nws):
    ...
    try:
      ...
      forecast = r2.json()
    except:							# If we get an error wait a minute and try again
      try:							# If not able to fech conditions then wait a minute and try again
        time.sleep(60)						
        r1 = requests.get(ApiUrl1)	  			# Fetch current conditions from the web 			
        conditions = r1.json()
        temp = int(float(conditions['current_observation']['temp_f'])) # Remove decimal point for all temperatures
...
def fetch_weather_timer(nws):
    global t
    fetch_weather(nws)
    t = threading.Timer(CHECK_DELAY, fetch_weather_timer, [nws]) # Wait X number of minutes before running this function again
    t.start()
...
def check_and_color_code_events():
    global active_weather_alert
    # LED options are solid|snooze|blink|die RRR GGG BBB intensity(0-100) RRR|GGG|BBB take values between 0-255
    if rain_tomorrow():
...
def display_events(LEDcommands, SolidLastLEDcmds):
    for line in range(len(LEDcommands)):             		# Cycle through all the pending LED commands
      if LEDcommands:						# Make sure there's at least one command to display
        if len(LEDcommands) != 1:				# If there's 2+ commands then display the LED seqence
        ...
        elif SolidLastLEDcmds != LEDcommands:			# If there's 2+ commands and command changed then display the solid_LED sequence
          if (LEDcommands[line][0] == "solid"): 		# If solid command encountered then display the solid_LED sequence
            solid_LED(int(LEDcommands[line][1]),int(LEDcommands[line][2]),int(LEDcommands[line][3]),int(LEDcommands[line][4]))
            SolidLastLEDcmds = LEDcommands
            time.sleep(2.0)
...
if __name__ == "__main__":
    ...
    fetch_weather_timer(nws)					# Run immediately and then every CHECK_DELAY seconds to see alert status 
    see_alerts = True
    ...
    while True:
      ...
      try: 
        ...
        if see_alerts == True:					# If see_alerts is enabled then...
          check_and_color_code_events()				# Check to see if there are any alerts
